...
class BaseInfoFetcher:
	def run(self, items, sql_file, web_fetcher):
		for fund in items:
			...
			html = web_fetcher.run(url, html_file)
...
if __name__ == "__main__":
	...
	is_initial = MyUtils.str2bool(config.get('initial', 'is_initial'))
	print is_initial
	file_path = config.get('path', 'file_path')+run_date
	thread_count = int(config.get('threads', 'thread_count'))
